...
import { useTexture, Loader, Environment, useFBX, useAnimations } from '@react-three/drei'
...
const host = 'https://avatar.stratech.srl:5000'
const path = 'https://avatar.stratech.srl:3000'
...
function Avatar({ speak, setSpeak, setAudioSource, playing, audio, setText }) {
  ...
  useEffect(() => {
    ...
    setText({ input: 'loading', output: 'loading' })
    makeSpeech(threadId, audio)
      .then(response => {
        if (Object.keys(response.data).length === 0) throw new Error('response data empty')
...
const MyAvatar = () => {
  ...
  return (
    <>
      ...
      <Loader />
      {text.input === 'loading' && <div style={{
        position: 'absolute', top: '0', bottom: '0', right: '0', left: '0', display: 'flex',
        justifyContent: 'center',
        alignItems: 'center'
      }}>
        <img style={{ height: "45dvh", width: "45dvh", objectFit: "cover" }} src="kOnzy.gif" alt="GIF" class="gif" />
      </div>}

      {!['', undefined, null].includes(text.input) &&
        <div style={{ position: 'absolute', top: '25px', right: '7%', zIndex: 500 }}>
          <div style={{ marginLeft: '25px', position: 'relative' }}>
            <div style={{
              ...
              {text.input === 'loading' ? 'In attesa di risposta' : text.input}
              <div style={{
                content: '""',
                position: 'absolute',
                top: '0px',
...
function Bg() {
  const texture = useTexture('/fondo.png')
